...
class CustomUserCreationForm(UserCreationForm):
  ...
  role = forms.ModelChoiceField(queryset=Role.objects.all(),disabled=True)
  ...
  program = forms.ModelChoiceField(queryset=Program.objects.all(),required=True, widget=forms.Select(attrs={'class':'form-select p-2'}))
  ...
  def __init__(self,*args,**kwargs):
    ...
    if self.dean:
      ...
      self.fields['role'].queryset = Role.objects.filter(name='Faculty')# sets the initiaal value of role to faculty
